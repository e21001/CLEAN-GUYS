<?php
  ...
  $page = max($page, 1);
  // 最終ページを取得する
  $counts = $db->query('SELECT COUNT(*) AS cnt FROM posts WHERE posts.category_id=1');
  $cnt = $counts->fetch();
  ...
  // 投稿を取得する
  $posts = $db->prepare('SELECT u.name, u.picture, p.* FROM users u, posts p WHERE u.id=p.user_id AND p.category_id=1 ORDER BY p.created DESC LIMIT ?, 10');
  ...
  <div class="container">
    ...
    <main class="post-page">
      <article class="">
        ...
        <ul class="paging">
          <?php if ($page > 1): ?>
            <li><a href="member.php?page=<?php print($page - 1) ?>">前のページへ</a></li>
          <?php else: ?>
            <li>前のページへ</li>
          <?php endif ?>
          <?php if ($page < $maxPage): ?>
            <li><a href="member.php?page=<?php print($page + 1) ?>">次のページへ</a></li>
          <?php else: ?>
            <li>次のページへ</li>
          <?php endif ?>
        </ul>
